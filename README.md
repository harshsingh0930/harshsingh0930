<div align="center">

# Hey, I'm Harsh 👋
### aka Bankai / Harsh Singh

<em>CS student • Systems & backend enthusiast • Open source tinkerer</em>

<br>

<p>
Interested in computer architecture, backend systems, full-stack development, data analytics, and experimenting with new LLMs.
</p>

<br>

<img src="https://raw.githubusercontent.com/harshsingh0930/harshsingh0930/master/info-card.svg" width="520" alt="Harsh's Info Card" />

<br><br>

<img src="https://raw.githubusercontent.com/harshsingh0930/harshsingh0930/master/contrib-heatmap.svg" width="860" alt="Harsh's GitHub Contribution Heatmap" />

<br><br>

<div align="center">
  <a href="https://github.com/harshsingh0930?tab=repositories">
    <img src="https://img.shields.io/badge/Projects-View%20Repos-24292f?style=for-the-badge&logo=github&logoColor=white" alt="View repositories" />
  </a>
</div>
</div>

---

## About Me

I'm a Computer Science student with a deep-rooted love for how machines work. My journey started with a Pentium G2020 and 4GB of DDR3 RAM, and that curiosity has only grown stronger over time.

- **Fascinated by:** GPUs, CPUs, TPUs, NPUs, and computer architecture in general
- **Currently improving:** Data Structures & Algorithms and backend development
- **Exploring next:** RAG pipelines and AI systems
- **Also into:** Full-stack development, data analytics, and experimenting with new LLMs

---

## Tech Stack

### Languages

![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![Java](https://img.shields.io/badge/JAVA-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Assembly](https://img.shields.io/badge/Assembly-black?style=for-the-badge)

### Tools & Platforms

![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![VSCode](https://img.shields.io/badge/VS%20Code-0078D4?style=for-the-badge&logo=visual-studio-code&logoColor=white)

---

## Currently Working On

- Sharpening DSA and quantitative aptitude
- Building backend projects with clean, scalable architecture
- Getting hands-on with RAG and AI system design

---

<div align="center">
<em>Thanks for stopping by — always up for a chat about architecture, LLMs, or good backend design.</em>
</div>
