...
# --- TIMING ADJUSTMENTS ---
START_DELAY = 3    # Wait 1.5s (time to scroll)
COL_T = 0.035       # Slower horizontal sweep
ROW_T = 0.080       # Slower vertical cascade
CELL_DUR = 0.65     # Slower fade-in
...
""".strip()
    ...
    parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{canvas_w}" height="{canvas_h}" viewBox="0 0 {canvas_w} {canvas_h}" font-family="ui-monospace,SFMono-Regular,Menlo,monospace">',
        f'<style>{css}</style>',
    ...
    for ci, column in enumerate(grid):
        for ri, cell in enumerate(column):
            ...
            parts.append(f'<rect class="c" x="{grid_left + ci * STEP}" y="{grid_top + ri * STEP}" width="{CELL}" height="{CELL}" rx="2.5" fill="{PALETTE[lvl]}" style="animation-delay:{delay:.3f}s"><title>{date_s}: {count} contributions</title></rect>')
